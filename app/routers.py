...
# def get_latest():
#     try:
#         with open("sensor_data/sensor_data.csv", newline="", encoding="utf-8") as f:
#             rows = list(csv.DictReader(f))

#             # lọc bỏ dòng rỗng hoặc thiếu giá trị
#             rows = [r for r in rows if r["Temp(C)"] not in (None, "", " ")]
...
#                 buckets["temp"][hour].append(float(row["Temp(C)"]))
#                 buckets["humid"][hour].append(float(row["Humidity(%)"]))
#                 buckets["press"][hour].append(float(row["Pressure"]))
#                 buckets["light"][hour].append(float(row["Light(lux)"]))
#                 buckets["gas"][hour].append(float(row["Gas"]))
